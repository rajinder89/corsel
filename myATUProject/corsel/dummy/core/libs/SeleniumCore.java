	...
	public static class seleniumHelper{
			...
			if(browserName.indexOf("chrome") != -1)
				{
					...
					try
					{
						if(Log.gsAutomationBrowserProfile != null)
						{
							...
							ATUReports.add("The Firefox Browser has been configured using the following user profile file: " + profile, LogAs.FAILED, null);
						...
						ATUReports.add("Error loading browser profile file found in gsAutomationBrowserProfile property in the Automation.properties file", LogAs.FAILED, null);
		...
		try {	
			...
			while ((driver.getTitle().isEmpty()) && (nCount < 10)) {
				...
				ATUReports.add("Error Loading URL: " + urlToLoad, LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
				
				String sException = e2.getMessage();
				ATUReports.add(sException, LogAs.FAILED, null);
		}
	}
